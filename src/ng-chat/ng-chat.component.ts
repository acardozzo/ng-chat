...
export class NgChat implements OnInit, IChatController {
  ...
  public audioSource: string =
    "https://raw.githubusercontent.com/rpaschoal/ng-chat/master/src/ng-chat/assets/notification.wav";
  ...
  private bootstrapChat(): void {
    ...
    if (!this.isBootstrapped) {
      ...
      if (initializationException) {
        console.error(
          `An exception has occurred while initializing ng-chat. Details: ${initializationException.message}`
        );
  ...
  fetchMessageHistory(window: Window) {
    ...
    if (this.adapter instanceof PagedHistoryChatAdapter) {
      ...
      this.adapter
        .getMessageHistory(window.participant.id)
        .pipe(
          map((result: Message[]) => {
            result.forEach((message) => this.assertMessageType(message));
  ...
  private emitMessageSound(window: Window): void {
    console.log([this.audioEnabled, window.hasFocus, this.audioFile]);
  ...
  private restoreWindowsState(): void {
    try {
      if (this.persistWindowsState) {
        const stringfiedParticipantIds = localStorage.getItem(
          this.localStorageKey
        );
        ...
        if (stringfiedParticipantIds && stringfiedParticipantIds.length > 0) {
          ...
          const participantsToRestore = this.participants.filter(
      ...
      console.error(
        `An error occurred while restoring ng-chat windows state. Details: ${ex}`
      );
    }
  }
  ...
  onWindowTabTriggered(payload: {
    ...
    let windowToFocus =
      this.windows[currentWindowIndex + (shiftKeyPressed ? 1 : -1)]; // Goes back on shift + tab

    if (!windowToFocus) {
      // Edge windows, go to start or end
      windowToFocus =
        this.windows[currentWindowIndex > 0 ? 0 : this.chatWindows.length - 1];
    }
  ...
  onWindowResendMessage(message: Message) {
    this.adapter.sendMessage(message);
  }
